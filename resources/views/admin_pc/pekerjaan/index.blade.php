            ...
            <div>
                ...
                <div class="grid grid-cols-2 md:grid-cols-5 gap-4">
                    ...
                    <div class="bg-white p-5 rounded-[1.5rem] shadow-sm border border-slate-100 flex flex-col items-center text-center hover:shadow-md transition-all hover:-translate-y-1 group">
                        ...
                        <h4 class="text-[10px] font-black text-slate-400 uppercase tracking-widest mb-1">Bidang</h4>
                        <p class="font-bold text-slate-800 text-sm line-clamp-1 w-full" title="{{ $job->business_sector }}">{{ $job->business_sector }}</p>
                        ...
                        <div class="md:col-span-5 space-y-2">
                            ...
                            <div class="relative group">
                                ...
                                <input type="text" name="search" value="{{ request('search') }}"
                                    ...
                                    placeholder="Cari Nama, Bidang, atau Jenis Usaha...">
                            </div>
                        </div>

                        {{-- Filter Dropdown --}}
                        <div class="md:col-span-4 space-y-2">
                            <label class="text-xs font-bold text-slate-500 uppercase tracking-wider ml-1">Filter Bidang Usaha</label>
                            <div class="relative group">
                                <div class="absolute inset-y-0 left-0 pl-4 flex items-center pointer-events-none">
                                ...
                                <select name="business_sector"
                                    ...
                                    <option value="{{ $j }}" {{ request('business_sector') == $j ? 'selected' : '' }}>{{ $j }}</option>
                                    @endforeach
                                </select>
                    ...
                    <table class="w-full text-left border-collapse">
                        <thead>
                            <tr class="bg-slate-50 text-slate-400 uppercase text-[10px] font-black tracking-[0.2em]">
                                ...
                                <th class="px-6 py-5 border-b border-slate-100">Bidang Usaha</th>
                                <th class="px-6 py-5 border-b border-slate-100">Jenis & Minat</th>
                            ...
                            <tr class="hover:bg-slate-50/60 transition-colors group">
                                ...
                                <td class="px-6 py-5">
                                    <div class="flex items-center gap-3">
                                        ...
                                        <span class="font-bold text-slate-700 text-sm">{{ $agt->business_sector }}</span>
                                    </div>
                                </td>
                                <td class="px-6 py-5">
                                    <div class="space-y-1">
                                        @if($agt->business_type)
                                        <div class="flex items-center gap-2">
                                            <span class="text-[10px] font-bold text-slate-400 uppercase tracking-wide w-10">Jenis:</span>
                                            <span class="text-xs font-bold text-slate-700">{{ $agt->business_type }}</span>
                                        </div>
                                        @endif
                                        @if($agt->business_interest)
                                        <div class="flex items-center gap-2">
                                            <span class="text-[10px] font-bold text-slate-400 uppercase tracking-wide w-10">Minat:</span>
                                            <span class="text-xs font-medium text-slate-600 italic">"{{ $agt->business_interest }}"</span>
                                        </div>
                                        @endif
                                        @if(!$agt->business_type && !$agt->business_interest)
                            ...
                            <tr>
                                <td colspan="5" class="px-8 py-12 text-center">
                                    <div class="flex flex-col items-center justify-center opacity-50">
                                        ...
                                        <p class="text-sm font-bold text-slate-500">Belum ada data bidang usaha yang tercatat.</p>
                                    </div>
                                </td>
                            </tr>
                            @endforelse
