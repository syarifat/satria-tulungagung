<x-app-layout>
    <x-slot name="header">
        <div class="flex justify-between items-center">
            ...
            <a href="{{ route('anggota.pengkaderan.index') }}" class="flex items-center gap-2 px-6 py-3 bg-slate-100 hover:bg-slate-200 text-slate-700 rounded-2xl text-xs font-bold uppercase tracking-widest transition-all">
        ...
        <div class="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 space-y-6">
            ...
            <div class="bg-white shadow-2xl overflow-hidden rounded-[2.5rem] border border-gray-100">
                <div class="p-8 md:p-10 space-y-8">
                    ...
                    {{-- Action Buttons --}}
                    <div class="pt-6 border-t border-gray-100 flex flex-col sm:flex-row items-center justify-center gap-4">
                        <a href="{{ route('anggota.pengkaderan.edit', $pengkaderan->id) }}" class="w-full sm:w-auto px-8 py-3 bg-orange-600 hover:bg-orange-700 text-white rounded-2xl text-sm font-bold uppercase tracking-widest text-center transition-all shadow-lg">
                            Edit Data
                        </a>
                        <form action="{{ route('anggota.pengkaderan.destroy', $pengkaderan->id) }}" method="POST" onsubmit="return confirm('Yakin ingin menghapus data ini?')" class="w-full sm:w-auto">
                            @csrf
                            @method('DELETE')
                            <button type="submit" class="w-full px-8 py-3 border-2 border-rose-200 text-rose-600 hover:bg-rose-50 rounded-2xl text-sm font-bold uppercase tracking-widest text-center transition-all">
                                Hapus Data
                            </button>
                        </form>
                    </div>
