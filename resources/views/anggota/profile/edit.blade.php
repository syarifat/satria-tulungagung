    ...
    <div class="py-10">
        <div class="max-w-5xl mx-auto px-4 sm:px-6 lg:px-8">
            ...
            <div class="bg-white shadow-2xl overflow-hidden rounded-[2.5rem] border border-gray-100">
                ...
                <form method="POST" action="{{ route('anggota.profile.update') }}" enctype="multipart/form-data" class="p-8 md:p-10 bg-white space-y-8">
                    @csrf
                    @method('PUT')
                            ...
                            <div class="space-y-5">
                                <div class="flex flex-col items-center gap-4 mb-6">
                                    <div class="relative group">
                                        @if($user->avatar)
                                        <img id="edit-avatar-preview" src="{{ $user->avatar }}" alt="Profile" class="h-24 w-24 rounded-full border-4 border-emerald-500 shadow-lg object-cover">
                                        @else
                                        <div id="edit-avatar-placeholder" class="h-24 w-24 rounded-full bg-emerald-500 flex items-center justify-center text-white text-3xl font-bold shadow-lg">
                                            {{ strtoupper(substr($user->nama, 0, 1)) }}
                                        </div>
                                        <img id="edit-avatar-preview" class="h-24 w-24 rounded-full border-4 border-emerald-500 shadow-lg object-cover hidden">
                                        @endif
                                        ...
                                        <input type="file" id="foto_profil" name="foto_profil" class="hidden" accept="image/*" onchange="previewEditImage(this)">
                                    </div>
                                ...
                                <div class="space-y-1.5">
                                    ...
                                    <input class="block w-full border-gray-200 bg-slate-50 focus:bg-white focus:border-emerald-500 focus:ring-4 focus:ring-emerald-500/20 rounded-xl shadow-sm py-3 px-4 text-sm font-medium transition-all @error('nama') border-rose-500 @enderror"
                            ...
                            <div class="space-y-5">
                                ...
                                <div class="space-y-1.5">
                                    ...
                                    <select id="tingkatan_organisasi" name="tingkatan_organisasi" onchange="toggleSatuan()" required class="block w-full border-gray-200 bg-slate-50 focus:bg-white focus:border-emerald-500 focus:ring-4 focus:ring-emerald-500/20 rounded-xl shadow-sm py-3 px-4 text-sm font-medium transition-all cursor-pointer">
                                        ...
                                        <option value="pac" {{ old('tingkatan_organisasi', $tingkatan) == 'pac' ? 'selected' : '' }}>Pimpinan Anak Cabang (Kecamatan)</option>
                                        <option value="pr" {{ old('tingkatan_organisasi', $tingkatan) == 'pr' ? 'selected' : '' }}>Pimpinan Ranting (Desa)</option>
                                    </select>
                                </div>
                                ...
                                <div class="space-y-1.5">
                                    ...
                                    <select name="jabatan_id" required class="block w-full border-gray-200 bg-slate-50 focus:bg-white focus:border-emerald-500 focus:ring-4 focus:ring-emerald-500/20 rounded-xl shadow-sm py-3 px-4 text-sm font-medium transition-all cursor-pointer">
                                        ...
                                        <option value="{{ $jab->id }}" {{ old('jabatan_id', $anggota->jabatan_id) == $jab->id ? 'selected' : '' }}>{{ $jab->nama }}</option>
                                        @endforeach
                                    </select>
                                </div>
                                ...
                                <div class="grid grid-cols-2 gap-4">
                                    ...
                                    <div id="wrapper_pac" class="hidden space-y-1.5">
                                        ...
                                        <select id="unit_kecamatan" name="unit_kecamatan_id" onchange="fetchDesa('unit_desa', this.value, true)" class="block w-full border-gray-200 bg-slate-50 focus:bg-white focus:border-emerald-500 focus:ring-4 focus:ring-emerald-500/20 rounded-xl shadow-sm py-3 px-4 text-sm font-medium transition-all cursor-pointer">
                                            <option value="" disabled>Pilih PAC...</option>
                                            @foreach($kecamatans as $kec)
                                            <option value="{{ $kec->id }}" {{ old('unit_kecamatan_id', $organisasiUnit?->kecamatan_id) == $kec->id ? 'selected' : '' }}>PAC GP Ansor {{ Str::title($kec->nama) }}</option>
                                            @endforeach
                                        </select>
                                    </div>
                                    ...
                                    <div id="wrapper_pr" class="hidden space-y-1.5">
                                        ...
                                        <select id="unit_desa" name="unit_desa_id" class="block w-full border-gray-200 bg-slate-50 focus:bg-white focus:border-emerald-500 focus:ring-4 focus:ring-emerald-500/20 rounded-xl shadow-sm py-3 px-4 text-sm font-medium transition-all cursor-pointer">
                                            <option value="" disabled>Pilih PAC Dulu...</option>
                                            @foreach($desasUnit as $desa)
                                            <option value="{{ $desa->id }}" {{ old('unit_desa_id', $organisasiUnit?->desa_id) == $desa->id ? 'selected' : '' }}>PR GP Ansor {{ Str::title($desa->nama) }}</option>
                                            @endforeach
                                        </select>
                                    </div>
                                </div>
                    ...
                    <div class="pt-8 border-t border-gray-100 flex flex-col sm:flex-row items-center justify-center gap-4 mt-4">
                        <a href="{{ route('anggota.dashboard') }}" class="w-full sm:w-auto px-8 py-3 border-2 border-slate-200 text-slate-500 rounded-2xl text-sm font-bold uppercase tracking-widest text-center hover:bg-slate-50 transition-all">
                            Batal
                        </a>
